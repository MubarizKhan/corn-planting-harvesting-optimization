# src/optimization/milp_scheduler_v2.py
...
def _compute_harvest_weather_factor(row: pd.Series) -> float:
    ...
    - Heavy rain  => big reduction
    - Cool temps  => some reduction
    - Low DDI     => poor drying (TMAX - TMIN small) => reduction
    - Low wind    => reduction

    Tune these thresholds/weights as needed.
    """
    factor = 1.0
...
def build_and_solve_schedule_v2(
    ...
    """
    Experimental MILP for corn planting/harvest scheduling with:
    ...
    # -----------------------
    m = gp.Model("corn_plant_harvest_schedule_v2")
